/*
+----------------------------PACBALL----------------------------+
|-> GRUPO: HERBERT RIBEIRO, GABRIEL RODRIGUES E FELIPE RODRIGUES|
|-> ETAPA 4 Versao 04.1 (mod. em 26/06/23)                      |
+---------------------------------------------------------------+
*/
...
int main(void)
{
    ...
    const char* window_title = "PACBALL GAME - SIN 141";
    const char* icon_img_filename = "./img/pacman-icon.png";
    const char* sprites_img_filename = "./img/sprites.png";
    const char* sample1_filename = "./sound/iniciojogo.wav";
    const char* sample2_filename = "./sound/pill.wav";
    const char* sample3_filename = "./sound/musicacopa1.wav";
    const char* sample4_filename = "./sound/eatsuperpill.wav";
    const char* sample5_filename = "./sound/finalcopa.wav";
    const char* sample6_filename = "./sound/errou.wav";
    ...
    while(!done)
    {
        ...
        while(playing)
        {
            ...
            else if(al_get_timer_count(timer) == 60)
            {
                if(!Char[INKY]->getActiveStat())
                {
            ...
            else if(al_get_timer_count(timer) == 80)
            {
                if(!Char[CLYDE]->getActiveStat())
                {
            ...
            // A dificuldade do jogo aumenta quando restam 10 dots no mapa, os fantasmas ficam mais rápidos.
            if(game_map.getDotCount() == 10 && !fastMode)
            {
                if(!mute)
                {
                    al_stop_sample(&siren_sound_id);
